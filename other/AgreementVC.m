...
- (void)viewDidLoad{
    ...
    lable2.text = @"    饥荒大事件提供的服务将严格按照本软件操作规则执行。用户必须确认所有服务条款并完成注册程序，才能成为饥荒大事件的正式用户。";
    ...
    lable4.text = @"    用户一旦注册成功，成为饥荒大事件的合法用户，将得到一个密码和用户名。 由用户对用户名和密码安全负全部责任。用户若发现任何非法使用用户帐号或存在安全漏洞的情况，请立即通告饥荒大事件。 饥荒大事件尊重用户个人隐私，未经合法用户授权，本软件不会在公开、编辑或透露其注册资料及保存在本网的非公开内容。";
    ...
    lable6.text = @"    用户个人对网络服务的使用自行承担风险。饥荒大事件所提供的所有资料权限于对用户参考，不对用户的商业运作做任何具体性指导，用户应自行承担使用或提供本网信息的商业活动及其风险。饥荒大事件不保证服务一定能满足用户的要求，也不保证服务不会受中断。本软件将尽力保证服务的及时性、准确性、安全性，但对及时性、准确性、安全性等都不作任何具体承诺。对用户在运用饥荒大事件得到的任何商品购物服务或交易进程，均不作任何担保。";
    ...
    lable8.text = @"    饥荒大事件对用户在接受服务过程中所受的任何直接、间接的损害不负责任，这些损害可能来自：不正当使用网络服务，在网上购买商品或进行同类型服务，在网上进行交易，非法使用网络服务或用户传送的信息有所变动等。 饥荒大事件不对用户所发布信息的删除或储存失败负责。饥荒大事件有判定用户的行为是否符合本网服务条款的要求和精神的保留权利，如果用户违背了服务条款的规定，饥荒大事件有中断对其提供网络服务的权利。 对用户自行提供的信息，由用户依法自行承担全部责任。饥荒大事件对此等信息的准确性、完整性、合法性或真实性均不承担任何责任。 用户在本软件所发表的任何意见均属于个人意见，并不代表饥荒大事件也持同样的观点。";
    ...
    lable12.text = @"    饥荒大事件服务条款的修改、服务变更、或其它重要事件发生变动而需要通告时，可根据实际情况选择通过重要页面公告、电子邮件、常规信件等形式进行。";
    ...
    lable14.text = @"    本网网络服务条款与中华人民共和国的法律解释相一致，用户和饥荒大事件一致同意服从中华人民共和国法律的管辖。如发生饥荒大事件服务条款与中华人民共和国法律相抵触时，则这些条款将完全按法律规定重新解释，而其它条款则依旧保持对用户产生法律效力和影响。";
